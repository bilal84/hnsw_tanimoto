...
public:
    ...
    RNNDescent(int ntotal, int L, int S, int T1, int T2, int random_seed,std::vector<int> keys, std::vector<std::vector<char>> new_fps)
        : ntotal(ntotal), L(L), S(S), T1(T1), T2(T2), random_seed(random_seed), keys(keys), new_fps(new_fps), has_built(false) {}
        ...
        #pragma omp parallel
        {
            ...
            for (int i = 0; i < ntotal; i++) {
                ...
                for (int j = 0; j < S; j++) {
                    int id = tmp[j];
                    if (id == i) continue;
                    double dist = new_fps_distance(keys[i], keys[id]);
        ...
        for (int u = 0; u < ntotal; ++u) {
            ...
            for (auto&& nn : old_pool) {
                ...
                for (auto&& other_nn : new_pool) {
                    ...
                    double distance = new_fps_distance(keys[nn.id], keys[other_nn.id]);
...
constexpr size_t NEW_FPS_BITS = 32;
...
public:
    ...
    void insert_list(const std::vector<int>& L) {
        ...
        for (int lc = top_layer; lc >= 0; --lc) {
            ...
            RNNDescent rnn(ntotal, 96, 20, 14, 15, 42, keys, new_fps);
