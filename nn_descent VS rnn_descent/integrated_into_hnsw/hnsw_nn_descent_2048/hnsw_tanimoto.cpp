...
namespace nng
{
	...
	public:
		...
		sim_value_type sim(const data_type &a, const data_type &b){
			...
			double result = tanimoto_similarity(fps[a], fps[b]);
	...
	public:
		NNDescent(const uint32_t K, std::vector<Fingerprint> fps, const double rho = 0.5, const double perturb_rate = 0, const std::size_t num_random_join = 10, const rand_type &rand = rand_type(0))
			: K(K), fps(fps), num_random_join(num_random_join), rho(rho), perturb_rate(perturb_rate), nodes(), nbds(), rand(rand), checked()
		{
		}

		NNDescent(const std::size_t K, std::vector<Fingerprint> fps, const std::vector<data_type> &datas, const double rho = 0.5, const double perturb_rate = 0, const std::size_t num_random_join = 10, const rand_type &rand = rand_type(0))
			: K(K), fps(fps), nodes(), num_random_join(num_random_join), rho(rho), perturb_rate(perturb_rate), nbds(), rand(rand), checked()
		{
			init_graph(datas);
		}
