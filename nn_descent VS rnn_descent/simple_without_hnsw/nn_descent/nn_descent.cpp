...
int main()
{
	...
	std::cout << "Total time for NNDescent updates : " << elapsed.count() << " seconds" << std::endl;
	...
	elapsed = end - start;
	std::cout << "Time for naive research : " << elapsed.count() << " seconds" << std::endl;

	std::cout << "\nNNDescent neighbors of the point " << index << " : ";
	std::unordered_set<int> nnDescentNeighbors;
	for (const auto &neighbor : nn.nbds[index])
	{
		std::cout << neighbor.node_id << " (" << -neighbor.similarity << "), ";
		nnDescentNeighbors.insert(neighbor.node_id);
	}
	std::cout << std::endl;

	std::cout << "\nNaive neighbors of the point " << index << " : ";
	int correctCount = 0;
	for (int i = 0; i < L; ++i)
	{
		std::cout << naive_neighbors[i].first << " (" << naive_neighbors[i].second << "), ";
	...
	std::cout << "\nPercentage of correct neighbors found by NNDescent : " << percentageCorrect << "%" << std::endl;
